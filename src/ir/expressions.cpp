...
bool Expression::matches(const ExpressionPtr& _other, const MatchesPtr& _pMatches) const {
    if (!_pMatches)
...
public:
    Substitute(Matches& _matches) :
        m_matches(_matches)
    {}
    virtual bool visitWild(const std::shared_ptr<ir::Wild>& _node) {
...
bool Expression::implies(const ExpressionPtr& _pLeft, const ExpressionPtr& _pRight) {
    if (_matches(_pLeft, _pRight, nullptr))
        return true;
    if (!_pLeft || !_pRight)
        return false;
